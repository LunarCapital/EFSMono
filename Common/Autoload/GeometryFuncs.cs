...
namespace EFSMono.Common.Autoload
{
    ...
    public class GeometryFuncs : Node
    {
        ...
        public static bool IsPolygonCCW(IReadOnlyList<Vector2> perimVertices)
        {
        ...
        public static float GetAreaOfPolygon(IReadOnlyList<Vector2> perimVertices)
        {
        ...
        private static float _GetRawAreaOfPolygon(IReadOnlyList<Vector2> perimVertices)
        {
        ...
        public static bool IsPolyInPoly(IReadOnlyList<Vector2> innerPoly, IReadOnlyList<Vector2> outerPoly)
        {
            if (innerPoly is null) throw new ArgumentNullException(nameof(innerPoly));
            if (outerPoly is null) throw new ArgumentNullException(nameof(outerPoly));

            if (GetAreaOfPolygon(innerPoly) >= GetAreaOfPolygon(outerPoly) ||
                ArePolysIdentical(innerPoly, outerPoly)) return false;

            for (int i = 0; i < innerPoly.Count; i++)
            {
                for (int j = 0; j < outerPoly.Count; j++)
                {
                    Vector2 innerVertexA = innerPoly[i];
                    Vector2 innerVertexB = innerPoly[(i + 1) % innerPoly.Count];
                    Vector2 outerVertexA = outerPoly[j];
                    Vector2 outerVertexB = outerPoly[(j + 1) % outerPoly.Count];
                    if (DoSegmentsIntersect(innerVertexA, innerVertexB, outerVertexA, outerVertexB))
                        return false;
                }
            }
            return IsPointInPoly(innerPoly[0], outerPoly);
        }
        ...
        public static bool IsPointInPoly(Vector2 point, IReadOnlyList<Vector2> poly)
        {
            if (poly is null) throw new ArgumentNullException(nameof(poly));
            return _WindingNumOfPointInPoly(point, poly) != 0;
        }
        ...
        private static int _WindingNumOfPointInPoly(Vector2 point, IReadOnlyList<Vector2> poly)
        {
            int windingNum = 0;
            for (int i = 0; i < poly.Count - 1; i++)
            {
                ...
                {
                    if (nextVertex.y > point.y && _IsLeft(thisVertex, nextVertex, point) > 0)
                        windingNum++;
                }
                else
                {
        ...
        private static float _IsLeft(Vector2 p0, Vector2 p1, Vector2 p2)
        {
        ...
        public static bool IsPointOnPolyBoundary(Vector2 point, IReadOnlyList<Vector2> poly)
        {
            if (poly is null) throw new ArgumentNullException(nameof(poly));
        ...
        public static bool ArePolysIdentical(IReadOnlyList<Vector2> polyA, IReadOnlyList<Vector2> polyB)
        {
            if (polyA is null) throw new ArgumentNullException(nameof(polyA));
            if (polyB is null) throw new ArgumentNullException(nameof(polyB));

            if (polyA.Count != polyB.Count) return false;
        ...
        public static Vector2[] SimplifyVectorArray(IReadOnlyList<Vector2> perim)
        {
            if (perim is null) throw new ArgumentNullException(nameof(perim));

            Vector2[] uniqueVertices;
            if (perim[0] == perim[perim.Count - 1])
            { //input follows closed array convention so we need to shave the last one off
                uniqueVertices = new Vector2[perim.Count - 1];
                for (int i = 0; i < uniqueVertices.Length; i++)
                {
        ...
        {
            if (perimeter is null) throw new ArgumentNullException(nameof(perimeter));

            if (perimeter.Count == 0) return -1;
            Vector2 startCoord = perimeter[0];
            int startID = 0;
